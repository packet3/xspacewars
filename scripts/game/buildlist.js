...
function Buildlist() {
	...
	time.textContent = GetRestTimeFormat(rest)
...
$(document).ready(function () {
	...
	//show remove build modal
	let demolishBuild = document.getElementsByClassName('break_build')
	for (let i = 0; i < demolishBuild.length; i++) {
		demolishBuild[i].addEventListener('click', function (e) {
			var elmHtml = e.target.getAttribute('data_tooltip_content')
			Swal.fire({
				html: elmHtml,
