        ...
        <div id="planet_select" style="margin: auto;left: 0;right: 0;top:46px;">
            ...
            <div class="palnet_row {if $current_pid == $ID}active_palnet_row{/if}">
				<div class="fleet_indicators">
                    ...
                    <img id="{$ID}m10" {if $Element.totalRockets == 0}style="display:none;"{/if} src="{$dpath}img/iconav/p_select_rocket.png" alt="" class="tooltip" data_tooltip_content="{$LNG.pla_attack_3}" />                 
                    {if $Element.luna !=0}  
                    <img id="{$Element.luna}m1" {if $Element.totalAttackLuna == 0}style="display:none;"{/if} src="{$dpath}img/iconav/p_select_moon_attack.png" alt="" class="tooltip" data_tooltip_content="{$LNG.pla_attack_4}" />
                    <img id="{$Element.luna}m6" {if $Element.totalRocketsLuna == 0}style="display:none;"{/if} src="{$dpath}img/iconav/p_select_moon_spio.png" alt="" class="tooltip" data_tooltip_content="{$LNG.pla_attack_5}" />       
                    <img id="{$Element.luna}m9" style="display:none;" src="{$dpath}img/iconav/p_select_destrued.png" alt="" class="tooltip" data_tooltip_content="{$LNG.pla_attack_6}" />
                ...
                <span class="{if $current_pid == $ID}active_urlpalnet{else}urlpalnet{/if}" url="cp={$ID}">
					...
					<span class="ico_build">
                        {if $Element.buildInfo.buildings}
                            <img src="{$dpath}img/iconav/p_select_build.png" alt="" class="tooltip" data_tooltip_content="
						...
						{if $Element.buildInfo.fleet}
                            <img src="{$dpath}img/iconav/p_select_ship.png" alt="" class="tooltip" data_tooltip_content="
                                <table class='reducefleet_table'>
                                    <tr>
						...
						{if $Element.buildInfo.tech}
							<img src="{$dpath}img/iconav/p_select_tech.png" alt="" class="tooltip" data_tooltip_content="
                                <table class='reducefleet_table'>
                                <tr>
        ...
        <div id="res_nav">
            {foreach $resourceTable as $resourceID => $resouceData} 
                {if !isset($resouceData.current)}
                    ...
                    <div id="res_block_{$resouceData.name}" class="bloc_res tooltip" data_tooltip_content="<span class='colore{$resourceID}'>{$LNG.tech.$resourceID}</span><div style='border-bottom:1px dashed #666; margin:7px 0 4px 0;'></div>{$LNG.RE} {$resouceData.percent|number}%">
                ...
                {else}
                    {if !isset($resouceData.current) || !isset($resouceData.max)}
                        <div id="res_block_{$resouceData.name}" class="bloc_res tooltip" data_tooltip_content="<span class='colore{$resourceID}'>{$LNG.tech.$resourceID}</span><div style='border-bottom:1px dashed #666; margin:7px 0 4px 0;'></div>{$LNG.RE} {pretty_number($resouceData.current)}">
                    ...
                    {else}
                        <div id="res_block_{$resouceData.name}" class="bloc_res tooltip" 
                            ...
                            <div class="ico_res"></div>
                            {/if}
                            {*<a href="game.php?page=trader&amp;mode=trade&amp;resource=901" class="exchange_res tooltip" data_tooltip_content="Обменять <span class='colore{$resourceID}'>{$LNG.tech.$resourceID}</span>"></a>*}
                            <div class="stock_res">
