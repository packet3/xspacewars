    ...
    <div id="content">
        ...
        {/if}
            <div class="fleettab7"> 
                ...
                <a href="game.php?page=reduceResources" class="fleet_reduce ico_reduceresources tooltip" data_tooltip_content="{$LNG.lm_reduceresources}"></a>
                {/if}
                {if isModuleAvailable($smarty.const.MODULE_REDUCE_FLEET)}
                <a href="game.php?page=reduceFleet" class="fleet_reduce ico_reducefleet tooltip" data_tooltip_content="{$LNG.lm_reducefleet}"></a>
                {/if}
                {if isModuleAvailable($smarty.const.MODULE_DELIVERY)}
                <a href="game.php?page=delivery" class="fleet_reduce ico_deliveryres tooltip" data_tooltip_content="{$LNG.lm_delivery}"></a>
                {/if}
                {if isModuleAvailable($smarty.const.MODULE_FLEET_TRADER)}
                <a href="game.php?page=fleetDealer" class="fleet_reduce ico_trader tooltip" data_tooltip_content="{$LNG.lm_fleettrader}"></a>
                {/if}
                {if isModuleAvailable($smarty.const.MODULE_FIND_DEBRIS)}
                <a href="game.php?page=findDebris" class="fleet_reduce ico_finddebris tooltip" data_tooltip_content="{$LNG.lm_finddebris}"></a>
                {/if}
            </div>
...
<div class="gray_flettab">
        <div class="transparent">
         ...
         <tr>
            ...
            <td><a class="tooltip" data_tooltip_content="<table width='100%'><tr><th colspan='2' style='text-align:center;'>{$LNG.fl_info_detail}</th></tr>{foreach $FlyingFleetRow.FleetList as $shipID => $shipCount}<tr><td class='transparent'>{$LNG.tech.{$shipID}}:</td><td class='transparent'>{$shipCount}</td></tr>{/foreach}</table>">{$FlyingFleetRow.amount}</a></td>
