...
<tr class="classificabarra ">
    ...
    <td class="classstat3">
        ...
        <a href="#" class="fbox-s-name classstat5" onclick="return Dialog.Playercard({$RangeInfo.id}, '{$RangeInfo.name}');">
            ...
            <img class="tooltip" data_tooltip_content="{$LNG.tech.{$RangeInfo.race}}" src="{$dpath}gebaeude/{$RangeInfo.race}.gif" width="18"</a>
            {/if}
            {if isModuleAvailable($smarty.const.MODULE_FORMGOVERNMENT)}
            <img class="tooltip" data_tooltip_content="{$LNG.tech.{$RangeInfo.formgovernment}}" src="{$dpath}gebaeude/{$RangeInfo.formgovernment}.png" width="18"</a>
            {/if}
            {if isModuleAvailable($smarty.const.MODULE_ETHICS)}
            <img class="tooltip" data_tooltip_content="{$LNG.tech.{$RangeInfo.ethics}}" src="{$dpath}gebaeude/{$RangeInfo.ethics}.png" width="18"</a>
            {/if}
